...
- [x] Use red for incorrect items (design: DESIGN-wrong-values.md; magic, enum, flags, floats, declared ranges landed 2026-09-18, table view cells and column counts the same day, WAV/AU/AIFF float samples declared finite; eager checksums still to do, so a bad checksum is still only in the inspector): a magic number that does not match the loaded template (visible right after switching template, without clicking), bad checksums, undefined enum values, numbers out of range, unexpected NaN/Inf/-Inf, unusual NaN payloads.
