...
- [ ] `proto4-sklearn-pipeline.pickle` hex view: at 0xe5 the SHORT_BINUNICODE opcode is a chip but the length+string after it (`\x05numpy`) gets no chip, so it is not selectable. Many similar cases. (2026-09-18: the opcode's chip now reads `SHORT_BINUNICODE numpy`, so the text is in the column; the operand is still part of that chip and not selectable on its own.)
