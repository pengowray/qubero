...
| xz | per block | stream and block headers; inside a block only as bytes. The LZMA2 in a block is read per symbol everywhere else, but an xz block may also carry delta and BCJ filters over it, and those are not undone here |
| FastLZ | per sequence | the token, the literals it carries, and each back-reference as `match(len, dist)` |
| lzip, LZMA1, LZMA2 | per symbol | the properties the container supplied, the five bytes that prime the range coder, each literal or `match(len, dist)`, the end-of-stream marker, and the bytes at the end no symbol ever pulled; lzip adds its header and trailer, and LZMA2 a step per chunk header and one per chunk stored outright. **A symbol's input range is the bytes the range decoder pulled while producing it, which is often none.** LZMA does not spend a whole number of bits on a symbol, so there is no per-bit map to draw: a reader standing on an output byte learns which input bytes were being consumed around it, and that is weaker than deflate's claim for a reason belonging to the format. The steps still tile the run exactly |
| bzip2 | per stream | one step over the whole run: its blocks are packed to the bit and the decoder will not say where one ended |
...
| RAR 5 | per symbol | each block's header, the code-length alphabet a nibble at a time with its runs of zeroes named as runs, the 430 lengths the four remaining tables are cut from, each literal or `match(len, dist)`, each filter definition, and the bits the last byte of a block was padded out with. The map from input bits to output *positions* is exact throughout; inside a range a filter rewrote, the step covering a byte names the literal or match that placed it before the filter ran. Solid, encrypted and split entries are not decoded and stay bytes |
