...
class HexPanel extends JPanel implements OpenFileListener, Scrollable, ResourceListener, DropTargetListener
{
	
	private boolean isOldSelection()
	{
		if (oldSelectionFirst == -1 || oldSelectionFirst > oldSelectionLast)
			return false;
	...
	public void resourceAdded(ResourceEvent e)
	{
		...
		if (e.getCategory().equals("Selection")) {
			...
			repaintAfterNewSelection(sm.getMinSelectionIndex(), sm.getMaxSelectionIndex());
		}
	...
	// active selection. added to openFile's
	private long activeSelectionFirst = -1;
	private long activeSelectionLast = -1;
    
	// when the selection is updated, know what to repaint
	private long oldSelectionFirst = -1;
	private long oldSelectionLast = -1;
	...
	{
		return (activeSelectionLast < activeSelectionFirst ? false : true);
    ...
    public HexPanel(OpenFile openFile)
	{
		...
		addMouseMotionListener(new MouseInputAdapter()
					...
					public void mouseDragged(MouseEvent e)
					{
						...
						{
							setSelection(activeSelectionFirst, hclick );
						}
	...
	public LongListSelectionModel getSelectionModelEditable()
	{
		//a selection model where changes are reflected on the hex panel. yeah right.
		return getSelectionModel(); //fixme
	}
	
	private void publishSelection()
	{
		//LongListSelectionModel
		openFile.setSelectionModel(getSelectionModel());
		activeSelectionFirst = -1;
		activeSelectionLast = -1;
		published = true;
	}
