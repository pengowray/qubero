...
/**
 * 
 * the logic of findRightOverlap() may be faulty. Test this.
 * 
 * @author Peter Halasz
 */
public class SegmentalBitSelectionModel { // implements BitSelectionModel
	...
	//everything but MULTIPLE_INTERVAL_SELECTION  is not supported
	...
	private static final DirectionalSegment EMPTY_SEGMENT = new DirectionalSegment(
			0, 0);

	//	contains the main selection data:
	private TreeSet<BitSegment> intervalSet = new TreeSet(); 
	//private TreeSet<BitSegment> intervalSet = new TreeSet(new firstComparator()); 
	...
	// cacheSet holds the intervalSet with the current selection fixed. access
	// it via: previewSaveActiveSelection()
	// should selecting and deselecting destroy the selection behind it? (if
	// done in one action)
	...
	public SegmentalBitSelectionModel(SegmentalBitSelectionModel model) {
		for (BitSegment s : model.getSegments()) {
			addSelectionInterval(s);
		}

		// fixme: take lead and anchor too. note above fixes model's active
		// selection in the new object

	}

	/**
	 * Change the selection to be the set union of the current selection and the
	 * indices between index0 and index1 inclusive. If this represents a change
	 * to the current selection, then notify each ListSelectionListener. Note
	...
	public void addSelectionInterval(BitSegment interval) {
		addSelectionInterval(new DirectionalSegment(	interval.lastIndex, interval.firstIndex));
	}
	 ...
	 * and the indices between index0 and index1 inclusive. If this represents a
	 * change to the current selection, then notify each ListSelectionListener.
	...
	/**
	 * returns the interval selection tree containing active selection, but does
	 * not save the active selection to the intervalSet
	 */
	...
	private TreeSet<BitSegment> saveActiveSelection() {
		if (!isIntervalsetUptodate) {
			if (isCacheValid) {
				intervalSet = (TreeSet) cacheSet.clone(); // FIXME:
																				// just
																				// assign
																				// without
																				// clone?
				...
				//System.out.println("saving active selection: " + activeSelection + " anti: " + activeAntiSelection);
				mergeInSelection(intervalSet);
			}

			activeSelection = EMPTY_SEGMENT;
			activeAntiSelection = EMPTY_SEGMENT;
			activeSelectionIsAnti = false; // fixme: should be
															// here? if not, add
															// to setAnchor
	...
	/*
	 * adds the selection interval but does not update lead/anchor, nor fires
	 * change event. returns segment over the changed range.
	 * 
	 * called only by previewSaveActiveSelection and saveActiveSelection, which
	 * check if it should be called and puts whichever treeset into the right
	 * state first.
	 */
	private void mergeInSelection(TreeSet tofix) {
		...
		if (activeSelection != EMPTY_SEGMENT) {
			...
			//System.out.println("leftOverlap: " + leftOverlap +" rightOverlap: " + rightOverlap);

			if (leftOverlap != null || rightOverlap != null) {
				BitCursor newStart = (leftOverlap == null ? seg.firstIndex
						: leftOverlap.firstIndex);
				BitCursor newEnd = (rightOverlap == null ? seg.lastIndex
						: rightOverlap.lastIndex);
	...
	/*
	 * returns a segment over the range that has been changed by the active
	 * segment only
	 */
	private BitSegment changeActiveSelectionInterval(
			DirectionalSegment interval, boolean anti) {
		...
		if (activeSelectionIsAnti != anti) {
			// changing from anti to non-anti or vice versa, so force fix old
			// selection
			saveActiveSelection();
		}
		...
		if (getSelectionMode() == SINGLE_SELECTION) {
			...
			newActive = new DirectionalSegment(interval.getHead(), interval
					.getHead().addBits(8));
			System.out.println("SINGLE_SELECTION not really supported");
		} else {
			newActive = interval;
			//System.out.println("changing new selection to:" + newActive + " from old:" + oldActive); 
		}
	...
	/**
	 * returns the segment (from intervalSet) to the left of seg which overlaps
	 * it (or just touches it if allowTouching is true). returns null if none
	 * found.
	 */
	private BitSegment findLeftOverlap(BitSegment objective,
		...
		if (justBefore.lastIndex
				.compareTo(objective.firstIndex.addBits(adjust)) < 0) {
			return null;
		}
	...
	/**
	 * returns the segment (from intervalSet) which overlaps it the right edge
	 * of seg (or just touches it if allowTouching is true). returns null if
	 * none found.
	 */
	private BitSegment findRightOverlap(BitSegment objective,
			boolean allowTouching) {
		
		//FIXME: i'm not sure if these values are correct!!
		int adjust = (allowTouching ? 1 : 0);
		
		BitCursor comparePoint = objective.lastIndex;
		...
		//FIXME: test this logic properly 
		if (justBefore.lastIndex.addBits(adjust).compareTo(comparePoint) < 0) {
			return null;
		}
	...
	public BitCursor getMinSelectionIndex() {
		 ...
		 * if (intervalSet.isEmpty() && activeSelection == EMPTY_SEGMENT &&
		 * activeAntiSelection == EMPTY_SEGMENT) return -1;
		 * 
		 * if (!activeSelectionIsAnti && !destructiveSelection) { if
		 * (intervalSet.isEmpty()) { return activeSelection.firstIndex; }
		 * 
		 * if (activeSelection == EMPTY_SEGMENT) { return
		 * ((Segment)intervalSet.first()).firstIndex; }
		 * 
		 * return Math.min(((Segment)intervalSet.first()).firstIndex,
		 * activeSelection.firstIndex); }
		 */
	...
	public long[] getSelectionIndexes() {
		/*
		 * //fixme: doesn't scale. array can "only" be Integer.MAX_VALUE in
		 * size.
		 * 
		 * TreeSet tree = previewSaveActiveSelection();
		 * 
		 * if (tree.isEmpty()) { return new BitSegment[0]; }
		 * 
		 * long[] array = new long[(int)getSelectionLength()]; int arraypos = 0;
		 * 
		 * for (Iterator i = tree.iterator(); i.hasNext();) { BitSegment seg =
		 * (BitSegment)i.next(); for (long ind = seg.firstIndex; ind <=
		 * seg.lastIndex; ind++) { array[arraypos++] = ind; } }
		 * 
		 * return array;
		 */
	...
	public void insertIndexInterval(long index, long length, boolean before) {
		...
		/*
		 * 
		 * long insMinIndex = (before) ? index : index + 1; long insMaxIndex =
		 * (insMinIndex + length) - 1;
		 *  // shift everything after the insertion to the right SortedSet
		 * shiftSet = intervalSet.tailSet(new Index(insMinIndex)); Set newSet =
		 * new TreeSet(); //FIXME: what type should this be optimially? Iterator
		 * i = shiftSet.iterator(); while (i.hasNext()) { BitSegment seg =
		 * (BitSegment)i.next(); BitSegment newSeg = new
		 * BitSegment(seg.firstIndex + length, seg.lastIndex + length);
		 * newSet.add(newSeg);
		 * 
		 * i.remove(); } intervalSet.addAll(newSet);
		 *  // shift part of the overhanging piece to the right BitSegment
		 * snipsnip = findLeftOverlap(new Index(insMinIndex), false); if
		 * (snipsnip != null) { BitSegment rightOfSnip = new
		 * BitSegment(insMaxIndex + 1, snipsnip.lastIndex + length);
		 * snipsnip.lastIndex = insMinIndex -1; }
		 * 
		 * if (anchorIndex >= index && anchorIndex <= index + length) {
		 * anchorIndex += length; }
		 * 
		 * if (leadIndex >= index && leadIndex <= index + length) { leadIndex +=
		 * length; }
		 * 
		 * activeSelection = moveSegment(activeSelection, index, length);
		 * activeAntiSelection = moveSegment(activeAntiSelection, index,
		 * length);
		 * 
		 * isCacheValid = false; //isIntervalsetUptodate = ... // unchanged
		 * 
	...
	private BitSegment moveSegment(BitSegment seg, long index, long length) {
		...
		/*
		 * if (seg == EMPTY_SEGMENT) return seg;
		 * 
		 * boolean moveSeg = false; BitCursor movetoFirst = seg.firstIndex;;
		 * BitCursor movetoLast = seg.lastIndex;;
		 * 
		 * if (seg.firstIndex >= index) { movetoFirst = seg.firstIndex + length;
		 * moveSeg = true; }
		 * 
		 * if (seg.lastIndex >= index) { movetoLast = seg.lastIndex + length;
		 * moveSeg = true; }
		 * 
		 * if (moveSeg) { return new BitSegment(movetoFirst, movetoLast); } else {
		 * return seg; }
		 */
	}

	/*
	 * // unused private Segment getActiveSelectionOrAnti() { if
	 * (activeSelectionIsAnti) { return activeAntiSelection; } else { return
	 * activeSelection; }
	 *  }
	 */
	...
	public boolean isSelectedIndex(BitCursor index) {
		/*
		 * if (index >= activeSelection.firstIndex && index <=
		 * activeSelection.lastIndex) { return true; }
		 */
		...
		SortedSet<BitSegment> headset = tree.headSet(new BitSegment(indexPlus,
				indexPlus));
	...
	public void setLeadSelectionIndex(BitCursor index) {
		...
		DirectionalSegment newActiveSel = new DirectionalSegment(index, getAnchorSelectionIndex());
	 ...
	 * <ul>
	 * <li><code>SINGLE_SELECTION</code> Only one list index can be selected
	 ...
	 * index") is used.
	 * <li><code>SINGLE_INTERVAL_SELECTION</code> One contiguous index
	 * interval can be selected at a time. In this mode setSelectionInterval and
	 * addSelectionInterval are equivalent.
	 * <li><code>MULTIPLE_INTERVAL_SELECTION</code> In this mode, there's no
	...
	public void setSelectionMode(int selectionMode) {
		...
		if (this.selectionMode == MULTIPLE_INTERVAL_SELECTION
				...
				&& intervalSet.size() >= 1) {
			/*
			 * //FIXME: keep only the set which lead or anchor lay on //for now
			 * just keep first segment Segment keep =
			 * (Segment)intervalSet.first(); SortedSet nokeep =
			 * intervalSet.tailSet(new Index(keep.firstIndex+1)); long
			 * firstChange = ((Segment)nokeep.first()).firstIndex; long
			 * lastChange = ((Segment)nokeep.last()).lastIndex;
			 * intervalSet.removeAll(nokeep); this.selectionMode =
			 * selectionMode; fireValueChanged(firstChange, lastChange);
			 */

			// clear everything but the active selection
			BitSegment change = new BitSegment(getMinSelectionIndex(),
					getMaxSelectionIndex());

			intervalSet.clear(); // deliberately don't clear active
											// selction
			...
			clearSelection(); // FIXME: may not fire anything if
										// selection is empty, so no
										// notification about lead change.
		} else {
			this.selectionMode = selectionMode;
		}
	...
	public Object clone() {
		try {
			SegmentalBitSelectionModel clone = (SegmentalBitSelectionModel) super
					.clone();
	...
	/*
	 * public SegmentalBitSelectionModel slice(BitSegment slice) { // start,
	 * length replaced with BitSegment BitSegment[] bits =
	 * getSelectionIndexes();
	 * 
	 * long index = start; long remain = length;
	 * 
	 * if (start < 0) { // skip negatives index = 0; remain = length + start; }
	 * 
	 * if (start >= bits.length) // start as after the end return new
	 * SegmentalBitSelectionModel(); // (-1,-1);
	 * 
