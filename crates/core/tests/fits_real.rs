...
fn a_real_tile_compressed_images_heap_reads_as_the_arrays_its_rows_point_at() {
    ...
    for row in 0..300usize {
        let count = ev.node(&doc, &at(&[1, row, 0, 0, 0])).unwrap().value.as_int().unwrap() as u64;
        let offset = ev.node(&doc, &at(&[1, row, 0, 0, 1])).unwrap().value.as_int().unwrap() as u64;
...
fn a_real_binary_tables_columns_are_typed_by_its_header() {
    ...
    let rows = ev.node(&doc, &at(&[2, 1])).unwrap();
    assert_eq!(rows.child_count, 2);
    assert_eq!(ev.node(&doc, &at(&[2, 1, 0])).unwrap().size_bits, 12 * 8);
    assert_eq!(ev.node(&doc, &at(&[2, 2])).unwrap().size_bits, 0);

    // `TFORM1 = '1J'`: one 32-bit integer, big-endian as everything here is.
    let c1 = ev.node(&doc, &at(&[2, 1, 0, 0])).unwrap();
    assert_eq!((c1.type_name.as_str(), c1.child_count), ("i32 be[]", 1));
    assert_eq!(ev.node(&doc, &at(&[2, 1, 0, 0, 0])).unwrap().value, Value::Int(1));
    assert_eq!(ev.node(&doc, &at(&[2, 1, 1, 0, 0])).unwrap().value, Value::Int(2));

    // `TFORM2 = '3A'`: three characters, read as one run of text.
    let c2 = ev.node(&doc, &at(&[2, 1, 0, 1])).unwrap();
    assert_eq!(c2.size_bits, 3 * 8);
    assert!(matches!(&c2.value, Value::Str(s) if s.trim() == "abc"), "{:?}", c2.value);

    // `TFORM3 = '1E'`: one float.
    assert_eq!(ev.node(&doc, &at(&[2, 1, 0, 2, 0])).unwrap().value, Value::Float(1.1));

    // `TFORM4 = '1L'`: a logical, written as the letter T or F; the first row says F.
    let c4 = ev.node(&doc, &at(&[2, 1, 0, 3])).unwrap();
    ...
    let widths: u64 = (0..4).map(|i| ev.node(&doc, &at(&[2, 1, 0, i])).unwrap().size_bits).sum();
