...
/// A descriptor as people write it: `0-12-101`.
...
/// A place in a run of bits, most significant first, which is how BUFR packs
/// everything. The same reader as [`grib_values`](super::grib_values)'s, and
/// a copy of it rather than a share, since that one is private to its module.
struct Bits<'a> {
    buf: &'a [u8],
    at: usize,
}
...
mod tests {
    ...
    fn change_width_and_scale_apply_to_numbers_and_not_to_counts() {
        ...
        let steps: Vec<&str> = r.steps.iter().map(|s| s.what.as_str()).collect();
        assert!(steps.iter().any(|s| s.starts_with("2-01-132 Change data width: +4 bits of width on 1 value")), "{steps:?}");
    }
    ...
    fn a_local_descriptor_with_no_width_stops_the_reading_and_says_why() {
        ...
        assert!(problem.contains("0-12-204") && problem.contains("local descriptor"), "{problem}");
