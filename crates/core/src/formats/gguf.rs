...
fn tensor() -> T {
    T::structure_named(
        ...
        vec![
            ...
            ("type", T::enumeration("GgmlType", T::u32(Little), ggml::GGML_TYPE)),
            ("offset", T::u64(Little)),
        ],
    )
}
...
pub fn gguf() -> Template {
    ...
    Template::new("gguf", root)
        ...
        // The shape is in the tensor table rather than in the weights
        // themselves, so both questions are answered from the record that
        // placed this child of the data section.
        .with_type(
            "Weights",
            ggml::weights(E::elem_field("tensors", E::idx(), &["type"]), &|| {
                E::product("tensors", E::idx(), &["dims"])
            }),
        )
}
