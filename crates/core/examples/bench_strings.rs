...
fn main() {
    for n in [25_000u64, 50_000, 100_000, 200_000] {
        ...
        let string = T::structure(
            "String",
            vec![("len", T::u64(Little)), ("text", T::utf8(E::field("len")))],
        );
        ...
        let cases: Vec<(i128, T)> = (0..13)
            ...
            .collect();
        let switched = T::array(
            T::switch(E::lit(8), cases, T::bytes(E::lit(0))),
            E::field("n"),
        );
        for (what, elem) in [("plain", plain), ("switched", switched)] {
            let t = Template::new(
                "t",
                T::structure("Root", vec![("n", T::u64(Little)), ("items", elem)]),
            )
