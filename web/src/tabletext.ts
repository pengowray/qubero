...
export function recordParts(i: number, row: TableRow, width: number, lead: Lead, fields?: FieldRange): Part[] {
  ...
  // The same two cells the address columns draw, `per cell` and `computed`
  // included: a pasted row that said `@0x40` and a length for a frame's row
  // would be claiming bytes the row does not hold.
  if (lead.addresses) {
    const [address, size] = copiedAddress(row);
    out.push({ text: address, numeric: false }, { text: size, numeric: false });
  }
  return out;
}
