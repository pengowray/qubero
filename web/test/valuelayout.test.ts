...
test("the piece of a value left on a row does not decide the layout", () => {
  ...
  const cells: Cell[] = [
    { index: 0, offset_bits: 14 * 8, size_bits: 24, text: "-394928", label: "-394928", kind: "int", contiguous: true, repeat: false, problems_within: [0, 0] },
  ];
...
test("a run nothing cuts is drawn at the hex pitch", () => {
  ...
  assert.equal(widestPieceBits({ index: 0, offset_bits: 120, size_bits: 32, text: "", label: "", kind: "int", contiguous: true, repeat: false, problems_within: [0, 0] }, 128), 24);
});
