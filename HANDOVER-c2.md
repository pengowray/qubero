...
2. ~~Render items virtualized; headings, rows, gaps, folds.~~ **Done**:
   ...
   sticky trail, which the virtualization note asks for. The drawing since moved to
   `web/src/listingdraw.ts`: one item in, one element out, reaching the view only
   through a `DrawContext` built once per paint. New item kinds draw there, not in
   the report.
